...
export const roadmapGroups: RoadmapGroup[] = [
  ...
  },
  {
    ...
    description:
      "지금은 프로덕션 URL 전체에 공유 비밀번호(HTTP Basic Auth) 하나만 걸려있고, 개인별 로그인이나 조직 구분은 없는 단일 사용자 데모 구조입니다. 회원 체계가 들어가면 이 공유 비밀번호 대신 여러 조직/여러 사용자가 각자 권한에 맞는 화면만 보게 됩니다.",
    items: [
      {
  ...
  },
  {
    ...
    trigger: "다중 조직 지원 시",
    description:
      "브랜드/카테고리/토픽/프롬프트/수집 실행 등 모든 상태는 이미 Postgres(Neon)로 이관되어 실 데이터베이스에 저장되고 있습니다. 다만 지금은 조직 하나(Neodigm)만 고정된 단일 테넌트 구조입니다 — 여러 조직을 지원하려면 아래 전환이 필요합니다.",
    items: [
      {
        id: "db-multi-tenant",
      ...
      },
      {
        id: "db-backup",
        title: "백업/PITR 정책 확정",
        description: "Neon 콘솔에서 백업 보관 기간·시점 복구(PITR) 설정을 실 운영 요구사항에 맞게 확인하고 필요하면 상위 플랜으로 조정합니다.",
      },
      {
        id: "db-collection-pipeline",
        title: "AI 수집 파이프라인의 서버 실행",
        description: "네이버/구글 AI 검색 수집은 봇 차단 회피를 위해 지금도 로컬 PC의 실제 브라우저로만 실행합니다. 이 파이프라인을 안정적으로 서버에서 돌리려면(예: 별도 워커/큐, 프록시 로테이션) 아키텍처 변경이 필요합니다 — 수집 결과는 로컬에서 실행한 뒤 명령어 한 줄로 프로덕션 DB에 반영합니다.",
      },
    ],
  },
  {
...
export const helpArticles: HelpArticle[] = [
  {
    ...
    content: [
      ...
      "왼쪽 메뉴는 크게 6개 영역으로 나뉩니다 — AI 가시성(수집된 데이터를 보는 곳), 프롬프트 관리(무엇을 추적할지 정하는 곳), 브랜드 관리(경쟁사 대비 가시성 비교), 도메인(우리 사이트 진단), 기회(개선할 항목과 수정 흐름), 설정(추적할 브랜드·카테고리 등록).",
      "가장 먼저 확인할 화면은 '개요'입니다. 콘텐츠 가시성, 가시성 점수, 브랜드 언급/인용 수, 최근 발견된 기회를 한눈에 보여줍니다.",
    ],
    highlights: ["왼쪽 메뉴 = 6개 영역", "첫 화면은 '개요'에서 시작", "각 화면 상단의 '어떻게 동작하나요' 배너를 먼저 읽어보세요"],
    relatedHref: "/",
  ...
  },
  {
    ...
    content: [
      ...
      "토픽 표의 '추이' 컬럼은 주차별 언급률을 작은 선그래프로 보여줍니다. 실행이 1건뿐인 토픽은 추세를 그릴 수 없어 대시(–)로 표시됩니다.",
    ],
    highlights: ["상위 프롬프트 vs 토픽 기회", "인용된 페이지/소스로 경쟁 구도 파악", "추이 컬럼으로 주차별 변화 한눈에 확인"],
    relatedHref: "/visibility-overview",
    relatedLabel: "가시성 개요 열기",
  },
  {
    slug: "collection-runs",
    title: "수집 로그로 AI 검색 결과 직접 모으기",
    category: "AI 가시성",
    summary: "키워드를 입력해 네이버 AI검색/구글 AI 모드 답변을 실제로 수집하고, 지금까지 모은 실행 목록을 확인합니다.",
    content: [
      "상단 폼에 키워드를 입력하고 엔진(네이버 AI검색/구글 AI 모드)을 선택해 '수집'을 누르면, 실제 브라우저가 그 검색을 실행해 AI 답변을 가져옵니다 — 봇 차단을 피하려고 사람이 검색하는 것과 똑같은 방식으로 동작해서 시간이 좀 걸립니다.",
      "진행 중에는 단계(설치 확인 → 수집 → 저장)가 실시간으로 표시되고, 구글 수집 중에는 실제 시크릿 크롬 창이 열립니다 — 자동으로 닫히니 직접 닫지 마세요.",
      "잘못 시작했거나 오래 걸린다면 '수집 중단' 버튼으로 즉시 멈출 수 있습니다 — 서버에서 실행 중인 프로세스를 실제로 종료합니다.",
      "수집이 끝나면 결과가 아래 '수집 실행 목록'에 즉시 반영되고, 행을 클릭하면 저장된 원문과 인용 소스를 볼 수 있습니다. 이 목록이 가시성 개요·브랜드 가시성 등 다른 화면의 실측 데이터의 원천입니다.",
      "이 페이지의 수집 기능은 실제 브라우저를 여는 방식이라 로컬 개발 환경에서만 동작합니다 — 프로덕션 사이트에서는 안내 메시지만 뜹니다. 새 결과를 프로덕션에 반영하려면 로컬에서 수집한 뒤 별도 명령으로 반영합니다.",
    ],
    highlights: ["실제 브라우저로 검색을 실행", "수집 중단으로 즉시 취소 가능", "결과가 다른 화면의 실측 데이터 원천", "실제 수집은 로컬 전용"],
    relatedHref: "/collection-runs",
    relatedLabel: "수집 로그 열기",
  },
  {
  ...
  },
  {
    ...
    content: [
      ...
      "'자사 인용 URL' 표는 원래 실제로 인용된 URL만 보여주지만, 아직 한 번도 인용되지 않은 새 콘텐츠도 상단 입력창으로 미리 등록해두면 0건인 채로 목록에 남아있습니다 — 인용되길 기다리는 콘텐츠를 놓치지 않고 추적할 수 있습니다. 필요 없어지면 '등록 해제'로 뺄 수 있습니다.",
    ],
    highlights: ["아직 인용 안 된 URL도 미리 등록해 추적 가능"],
  ...
  },
  {
    ...
    content: [
      "'기회'는 온사이트 콘텐츠 최적화(복잡도/FAQ/목차/멀티미디어/콘텐츠 가시성 회복/구조화 데이터)와 온사이트 기술 최적화(robots.txt)로 나뉩니다. 각 카드의 오른쪽 상태값이 지금 몇 개 URL이 수정 필요한지 보여줍니다.",
      "'구조화 데이터(JSON-LD) 추가'는 schema.org 구조화 데이터가 있는지 확인하는 기회입니다 — 구조화 데이터가 있으면 AI 크롤러가 페이지의 종류·속성을 더 명확하게 파악할 수 있습니다. 사이트맵을 다시 크롤링하면 이 값도 함께 갱신됩니다.",
      ...
      "'토픽 기회' 상세 페이지에는 Visibility Funnel이 함께 표시됩니다 — 전체 실행 중 AI가 실제로 답변을 만들었는지(AI 답변 존재)부터 자사 언급까지 4단계로 좁혀가며 어디서 새는지 보여줘서, '미노출'의 원인이 질문 자체의 문제인지 콘텐츠 문제인지 구분하는 데 도움을 줍니다.",
    ],
    highlights: [
      "카드 → URL 목록 → 재색인으로 검증",
      "구조화 데이터(JSON-LD)도 기회로 진단",
      "Visibility Funnel로 미노출 원인 구분",
      "제외/다시 포함으로 목록 정리",
      "선택/전체 재색인으로 일괄 검증",
    ],
  ...
  },
  {
    ...
    content: [
      ...
      "'브랜드 별칭'은 AI 답변에서 우리 브랜드를 다르게 부르는 표기(예: 네오디지엠, Neodigm)를 모아 같은 언급으로 집계되게 합니다. 직접 하나씩 추가해도 되고, 'AI로 별칭 최적화' 버튼으로 LLM에게 물어본 결과를 한 번에 반영할 수도 있습니다.",
      "'추적할 기타 브랜드'는 경쟁사입니다. '가시성 개요에서 추가'를 누르면 실제 수집된 AI 답변에서 언급된 브랜드 중 아직 추적하지 않는 것만 골라서 추가할 수 있고, 각 브랜드 이름을 누르면 그 브랜드의 별칭(다른 표기)도 관리할 수 있습니다.",
      "카테고리 섹션의 '프롬프트 수'는 실제 프롬프트 라이브러리에 있는 프롬프트 개수를 그대로 보여줍니다.",
      "별칭·기타 브랜드를 바꾸면 과거 화면 숫자도 바로 바뀔 수 있습니다 — 이미 수집해둔 AI 답변 원문 자체는 그대로지만, 언급/인용 분석은 최신 별칭 목록 기준으로 다시 계산되기 때문입니다. 그래서 오늘 별칭을 추가하면 지난주 그래프의 언급 수도 함께 늘어날 수 있습니다 — 오류가 아니라 의도된 동작입니다.",
    ],
    highlights: ["대기 중 → 활성 전환", "별칭 관리로 언급 집계 정확도 향상", "가시성 개요에서 경쟁사 바로 추가", "사이트맵 크롤로 콘텐츠 가시성 측정", "별칭 변경 시 과거 화면 숫자도 함께 재계산됨"],
  ...
  },
  {
    ...
    title: "가시성 숫자는 이렇게 읽으세요",
    category: "AI 가시성",
    summary: "언급률·인용률 같은 숫자가 왜 매번 조금씩 흔들리는지, 무엇과 비교해서 판단해야 하는지 정리했습니다.",
    content: [
      "생성형 AI는 같은 질문에도 매번 조금씩 다르게 답합니다. 어제는 우리 브랜드가 언급됐다가 오늘은 빠지기도 합니다 — 이건 오류가 아니라 AI 자체의 성질입니다. 그래서 하루치 숫자가 아니라 몇 주간의 추이로 판단해야 합니다.",
      "직접 ChatGPT나 다른 AI에게 같은 질문을 던져보고 '우리 브랜드가 나오는데 왜 화면엔 낮게 나오냐'고 느낄 수 있습니다. 질문 문구, 로그인 계정, 지역, 시점이 전부 다르면 AI 답변도 달라집니다. 화면의 숫자는 등록된 프롬프트 세트를 반복 수집해 얻은 값이고, 직접 해본 한 번은 표본 1건일 뿐이라 서로 달라도 정상입니다.",
      "'언급 수 30%면 잘하는 건가요?'에는 업종·주제마다 다른 절대적인 정답이 없습니다. 대신 세 가지 상대적 기준으로 보세요: ①자사 지표의 추이(지난 기간 대비 오르고 있는지가 가장 신뢰할 수 있는 기준입니다) ②경쟁사 대비 위치(마켓 트래킹에서 우리 절대값이 낮아도 경쟁사가 더 낮으면 그 주제에선 앞서 있는 것입니다) ③토픽 상세의 Visibility Funnel에서 'AI 답변 존재' 비율이 낮은지(이게 낮으면 언급이 적은 원인이 콘텐츠가 아니라 그 질문 자체에 AI가 답을 잘 안 만드는 것입니다).",
      "별칭·기타 브랜드 목록을 바꾸면 지난 기간의 화면 숫자도 함께 바뀔 수 있습니다 — 실제로 수집해둔 AI 답변 원문은 그대로지만, 언급/인용 분석 자체가 최신 별칭 목록 기준으로 다시 계산되기 때문입니다. '어제까지의 숫자가 왜 갑자기 바뀌었지'는 대부분 이 경우입니다.",
      "숫자 변화의 원인을 성급하게 콘텐츠 개선 덕분이라고 단정하지 마세요. AI 모델 자체가 업데이트되면 우리가 아무것도 안 해도 언급률이 움직일 수 있습니다. 같은 기간에 다른 변화(수집 엔진 추가, 프롬프트 세트 변경 등)가 없었는지 먼저 확인하세요.",
    ],
    highlights: [
      "하루 숫자보다 몇 주간 추이",
      "직접 물어본 결과와 화면 숫자는 원래 다를 수 있음",
      "판단은 자사 추이·경쟁사 대비·AI 답변 존재율 3가지 기준으로",
      "별칭 변경 시 과거 화면 숫자도 함께 재계산됨",
    ],
  },
  {
    slug: "pricing",
